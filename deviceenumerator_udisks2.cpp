...
QStringList DeviceEnumerator_udisks2::getRemovableDeviceNames() const
{
    ...
    {
        qDebug() << "block device path:" << blockDevice.path();
        ...
        qDebug() << "drive path:" << replyDrive.value().value<QDBusObjectPath>().path();
        ...
        qDebug() << "removable:" << replyRemovable.value().toBool();
...
QStringList DeviceEnumerator_udisks2::getUserFriendlyNames(const QStringList &devices) const
{
    ...
    qDebug() << "friendlyNames:" << friendlyNames.size();
